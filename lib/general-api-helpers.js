...
async function createGroupStageMatches(seasonNumber, tournamentId) {
  ...
  let scheduledTime;
  const matchObjs = placeholderTeams.reduce((matchObjs, team, i) => {
    // each team should have 9 matches against all other teams
    // for a total of 45 Match objects

    const allMatchesForTeam = placeholderTeams.map((comparedTeam, j) => {
      ...
      if (scheduledTime) {
        ...
        scheduledTime = new Date(new Date().getFullYear() + 1, 0, 1); // set to Jan 1st of next year
      }
