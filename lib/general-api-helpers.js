...
const { Match, MatchRound, Team, PlayerTeamHistory } = sequelize.models;
...
async function createGroupStageMatches(seasonNumber, tournamentId) {
  ...
  await MatchRound.bulkCreate(matchRoundObjs);
}
...
async function seedPlayoffs(season) {
  console.log(`seeding playoffs for season ${season}`);

  const seasonalGroupStageMatches = await Match.findAll({
    where: {
      season,
  ...
  console.log("seasonalGroupStageMatches", seasonalGroupStageMatches);

  // get sorted list of first to last place teams
  ...
  seasonTeams = seasonTeams.map((team) => {
    ...
    const groupStageWins = seasonalGroupStageMatches.filter(
      (m) => m.matchWinnerTeamId === team.id
    );
    const groupStageLosses = seasonalGroupStageMatches.filter(
  ...
  seasonTeams.sort((a, b) => {
    const teamAMatchSum = a.groupStageWins.length - a.groupStageLosses.length;
    const teamBMatchSum = b.groupStageWins.length - b.groupStageLosses.length;
  ...
  // hard code each placement without tiebreakers to bracket -------
  ...
  // add head-to-head tiebreaker calculations -------

  // add strength of victory tiebreaker calculations -------

  // add average duration tiebreaker calculations -------
}
