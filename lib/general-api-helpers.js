...
async function parsePlayerStats(
  ...
  const matchRoundPlayerStatsRecords = allPlayers.map((player, i) => {
    ...
    // todo:
    // ------ ADD ALL THESE TO SEQUELIZE PLAYER MODEL ------

    // find kill participation % (done)
    // find champion damage share (done)
    // find wards placed (done)
    // find wards destroyed (done)

    // find all end game stats from timeline AI
    // - ad
    // - ap
    // - attack speed
    // - life steal
...
module.exports = { getIdParam, parseTeamStats, parsePlayerStats };
